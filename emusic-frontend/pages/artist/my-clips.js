...
export default function MyClips() {
  ...
  const fetchClips = async () => {
    setLoading(true);
    try {
      const r = await api.get('/videos/feed?limit=100');
      const all = r.data || [];
      // Фильтруем только свои — по artist_id через профиль артиста
      const meRes = await api.get('/users/me');
      const me = meRes.data;
      // Получим artist_id через отдельный запрос если нужно
      const artistRes = await api.get('/artists/me').catch(() => null);
      const myArtistId = artistRes?.data?.id;
      const mine = myArtistId ? all.filter(v => v.artist_id === myArtistId) : [];
      setClips(mine);
    } catch {
      setClips([]);
    } finally {
